...
uint16_t ReadColourSensor(void)
{
  ...
  for (uint8_t desiredColor = 0; desiredColor < DIFFERENTCOLORS; desiredColor++)
  { //Read Red Green and Blue apart
    for (uint8_t i = 0; i < AVERAGECOLORS; i++)
    { //sample one color 5 times and take the average
  ...
  if (redColor < blueColor && redColor < greenColor && redColor < 20)
  { //color is red
    DEBUG_OUT("color is ORANGE\n\r");
    return 4; //4 equals orange in blockly
  }
  else if (blueColor < redColor && blueColor < greenColor && greenColor > 210)
  { //color is blue
...
uint8_t FrequencyToColourVal(uint16_t freq, uint8_t color)
{
  ...
  {
    return 0;
  } //bad frequency, don't return a color
